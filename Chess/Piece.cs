...
namespace Chess
{
    ...
    public class King : Piece
    {
        ...
        public List<Position> AttackedFields()
        {

            List<Position> attackedPositions = new List<Position>();

            foreach (Piece piece in Game.PiecesOnBoard.Where(p => p.Color != this.Color))
            {
                attackedPositions.AddRange(piece.PossibleMoves().FindAll(p1 => this.Position.Equals(p1)));
            }
